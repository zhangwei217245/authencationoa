...
public class UnicodeInputStream extends InputStream {
    ...
    public void init() throws IOException {
        ...
        //Edit by X-Spirit
        //如果之前猜测的字符集属于UTF类型，但是和本程序猜测结果不符，则编码类型设为默认的NULL
        unread = n;
        if ("UTF-32BE".equals(defaultEnc) || "UTF-32LE".equals(defaultEnc) || "UTF-16LE".equals(defaultEnc) || "UTF-16BE".equals(defaultEnc)) {
            if (Utility.isNotEmpty(encoding) && encoding.equals(defaultEnc)) {
                encoding = defaultEnc;
            } else {
                encoding = null;
            }
