...
Zapatec.Calendar.setup = function (params) {
	...
	function onSelect(cal) {
		...
		if (update && p.inputField) {
			var curTime = cal.currentDate.print(p.ifFormat);
			if(p.ifFormat.indexOf("%H:%M:%S") > -1){
					curTime = cal.currentDate.print(p.ifFormat).substring(0 ,cal.currentDate.print(p.ifFormat).indexOf(" ")) + new Time().get();
				}
			p.inputField.value = curTime;
